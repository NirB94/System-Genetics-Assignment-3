...
def association_model(genotypes_df, expression_df, alpha=0.05):
    ...
    genotypes_df = genotypes_df[genotypes_df.columns[:4].to_list() + relevant_cols]
    strains_dict = dict()
    for i in range(len(expression_df)):
        ...
        for index in genotypes_df.index:
            ...
            p_vals.append(reg.regression(phe, gen, False))
        strains_dict[phe['data']] = p_vals
    raw_mat = pd.DataFrame(strains_dict)
    raw_mat = raw_mat.T
    print(raw_mat.shape)
    raw_mat.index = expression_df['data']
    raw_mat.columns = genotypes_df.index
    raw_mat.drop(raw_mat.columns[0], axis=1, inplace=True)
